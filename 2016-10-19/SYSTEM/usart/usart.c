...
void uart5_init(u32 bound)
{
   ...
   //Usart1 NVIC 配置
    ...
    NVIC_InitStructure.NVIC_IRQChannel = UART5_IRQn;
	  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=3 ;//抢占优先级3
	  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;		//子优先级3
		NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;			//开启IRQ通道使能
		NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器
...
void UART5_IRQHandler(void)                	             //串口5中断服务程序
{
	//-----------------单片机接受程序----------------------------------//
		...
		{
		  Recc_byte =USART_ReceiveData(UART5);                  //(UART5->DR);	//读取接收到的数据
			if (UART5_RX_STA<UART5_REC_LEN)
			{
			UART5_RX_BUF[UART5_RX_STA]=Recc_byte;
			UART5_RX_STA++;
			}
			else
			{
...
void UART4_IRQHandler(void)                	             //串口4中断服务程序
{
	//-----------------单片机接受程序----------------------------------//
		...
		{
		  Recc_byte =USART_ReceiveData(UART4);                  //(UART4->DR);	//读取接收到的数据
			if (UART4_RX_STA<UART4_REC_LEN)
			{
			UART4_RX_BUF[UART4_RX_STA]=Recc_byte;
			UART4_RX_STA++;
			}
			else
			{
...
void USART3_IRQHandler(void)                	             //串口2中断服务程序
{
  ...
  // 		{
  // 		 Rec_byte =USART_ReceiveData(USART2);                  //(USART2->DR);	//读取接收到的数据
  // 			rxdata_com_zhuji[USART2_RX_STA++]=Rec_byte;
  // 		}	
	//---------------------接受NMEA0183数据-----------------------------------//	
		...
		if((USART3_RX_STA&0x8000)==0)//接收未完成
		{
			if(USART3_RX_STA&0x4000)//接收到了0x0d
			{
					...
					else 
					{
							...
							{
								rec_num++;
								USART3_RX_STA=USART3_RX_STA&0xBFFF;
							}
					}
			}
			else //还没收到0X0D
			{	
					...
					{
							USART3_RX_BUF[USART3_RX_STA&0X1FFF]=res;
							USART3_RX_STA++;
